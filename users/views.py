...
def signupprovider(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # If this is an API, you could return a JsonResponse instead
...
def providersignup(request):
    # return(request,'provider/provider_signup.html')
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            if user_profile.is_provider:
                ...
                request.session['user_profile_id'] = user_profile.id
                # Store user_profile_id in session for future use
                #request.session['user_profile_id'] = user_profile.id
...
#     print(f"Hourly Salary: {booking.provider.hourly_salary}")

#     # Calculate total amount based on hourly salary
#     total_amount = total_hours * booking.provider.hourly_salary
